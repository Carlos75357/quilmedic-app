...
class ProductoDetalleBloc extends Bloc<ProductoDetalleEvent, ProductoDetalleState> {
  ...
  late final HospitalRepository hospitalRepository = HospitalRepository(
    apiClient: apiClient,
  );
  late final ProductoRepository productoRepository = ProductoRepository(
    apiClient: apiClient,
  );
  ...
  Future<void> _onCargarHospitales(
    ...
    try {
      ...
      emit(
        ErrorCargaHospitalesState(
          'Error al cargar hospitales: ${e.toString()}',
        ),
      );
    }
  }
  ...
  Future<void> _onTrasladarProducto(TrasladarProductoEvent event, Emitter<ProductoDetalleState> emit) async {
    ...
    try {
      ...
      try {
        ...
        emit(
          ErrorTrasladoProductoState(
            'Error al verificar la conectividad: No se pudo verificar la conexión a internet',
          ),
        );
        return;
      }
      ...
      if (!hayConexion) {
        emit(
          ErrorTrasladoProductoState(
            'No hay conexión a internet para realizar el traslado. Intente nuevamente cuando tenga conexión.',
          ),
        );
        return;
      }

      final verificacionResponse = await productoRepository
          .getProductoByNumeroAndAlmacen(event.productoId, event.nuevoHospitalId);

      if (verificacionResponse.success) {
        emit(
          ProductoTrasladadoState(
            verificacionResponse.message ??
                'El producto ya se encuentra en el almacén especificado',
          ),
        );
        return;
      }
      ...
      if (e.toString().contains('SocketException') ||
        ...
        emit(
          ErrorTrasladoProductoState(
            'Error al trasladar producto: Ocurrió un problema durante el proceso',
          ),
        );
      }
    }
