...
class _ProductStockBadgeState extends State<ProductStockBadge> {
  ...
  Future<void> _loadStockMinimo() async {
    try {
      ...
      final hasSpecific = await widget.alarmUtils.hasSpecificAlarms(
        widget.productId,
      );

      if (hasSpecific) {
        // Obtener las alarmas específicas para este producto
        final alarms = await widget.alarmUtils.getSpecificAlarmsForProduct(
          widget.productId,
        );
        ...
        for (var alarm in alarms) {
          if (alarm.type?.toLowerCase() == 'stock' &&
              (alarm.locationId == null ||
                  alarm.locationId == widget.locationId)) {
  ...
  /// y luego muestra solo el stock actual (sin el stock mínimo)
  @override
